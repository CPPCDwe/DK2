...
MagicTulevo.AnimationSettings = {
    ...
    EnableSounds = true,          -- UI sounds
    EnableParticles = true,       -- Particle effects (typing, etc.)
    AnimationSpeed = 1.0,         -- Animation speed multiplier (0.5 = slow, 2.0 = fast)
}
    ...
    MainAnimationConnection = RunService.RenderStepped:Connect(function(dt)
        ...
        for gradient, data in pairs(AnimationQueue.GradientOffsets) do
            if gradient and gradient.Parent then
                data.offset = (data.offset + dt * data.speed * speedMult) % 1
        ...
        for object, data in pairs(AnimationQueue.Rotations) do
            if object and object.Parent then
                data.rotation = (data.rotation + dt * data.speed * speedMult) % 360
...
function MagicTulevo:CreateWindow(config)
    ...
    UI.MinimizeBtnContainer = Create("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(0, 32, 0, 32),
        ClipsDescendants = false,
        Parent = HeaderButtons
    })
    UI.MinimizeBtn = Create("TextButton", {
        BackgroundColor3 = Theme.Card,
        Size = UDim2.new(0, 32, 0, 32),
        Font = Enum.Font.GothamBold,
        Text = "",
        ...
        TextSize = 16,
        AutoButtonColor = false,
        Parent = UI.MinimizeBtnContainer
    })
    Create("UICorner", {CornerRadius = UDim.new(0, 8), Parent = UI.MinimizeBtn})
    Create("UIStroke", {Color = Theme.Border, Thickness = 1, Transparency = 0.7, Parent = UI.MinimizeBtn})
    
    -- Minimize Icon (dash/minus)
    UI.MinimizeIcon = Create("TextLabel", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 1, 0),
        Font = Enum.Font.GothamBold,
        Text = "—",
        TextColor3 = Theme.TextMuted,
        TextSize = 18,
        Parent = UI.MinimizeBtn
    })
    
    -- Minimize Tooltip
    UI.MinimizeTooltip = Create("Frame", {
        BackgroundColor3 = Theme.Secondary,
        Size = UDim2.new(0, 0, 0, 26),
        Position = UDim2.new(0, -6, 0.5, 0),
        AnchorPoint = Vector2.new(1, 0.5),
    ...
    Create("UICorner", {CornerRadius = UDim.new(0, 6), Parent = UI.MinimizeTooltip})
    Create("UIStroke", {Color = Theme.Accent, Thickness = 1, Transparency = 0.5, Parent = UI.MinimizeTooltip})
    Create("TextLabel", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, -10, 1, 0),
        Position = UDim2.new(0, 5, 0, 0),
    ...
    UI.MinimizeBtn.MouseLeave:Connect(function()
        Tween(UI.MinimizeBtn, 0.2, {BackgroundColor3 = Theme.Card})
        Tween(UI.MinimizeIcon, 0.2, {TextColor3 = Theme.TextMuted})
        Tween(UI.MinimizeTooltip, 0.2, {Size = UDim2.new(0, 0, 0, 26)})
    end)
    
    -- Minimize Button Click - Hide/Show window
    UI.MinimizeBtn.MouseButton1Click:Connect(function()
        PlaySound("rbxassetid://6895079853", 0.3)
        Window.Visible = false
        
        -- Save settings before minimizing
        local toggleStates = {}
    ...
    ApplyTheme = function(themeData)
        ...
        if UI.MinimizeBtn then UI.MinimizeBtn.BackgroundColor3 = Theme.Card end
        ...
        if UI.MinimizeIcon then UI.MinimizeIcon.TextColor3 = Theme.TextMuted end
        ...
        if UI.MinimizeTooltip then UI.MinimizeTooltip.BackgroundColor3 = Theme.Secondary end
        ...
        -- Update FOV circle color if exists
        if FOVCircle then
        ...
        -- Update Settings/Info/Configs tab content colors
    ...
    -- Animation Settings Section Label
    Create("TextLabel", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 0, 18),
        Font = Enum.Font.GothamBold,
    ...
    local AnimSettingsContainer = Create("Frame", {
        BackgroundColor3 = Theme.Card,
        Size = UDim2.new(1, 0, 0, 200),
        LayoutOrder = 2.6,
        Parent = PanelState.SettingsTabContent
    })
    Create("UICorner", {CornerRadius = UDim.new(0, 10), Parent = AnimSettingsContainer})
    Create("UIStroke", {Color = Theme.Border, Thickness = 1, Transparency = 0.5, Parent = AnimSettingsContainer})
    
    local AnimSettingsLayout = Create("UIListLayout", {
        Padding = UDim.new(0, 4),
        SortOrder = Enum.SortOrder.LayoutOrder,
        Parent = AnimSettingsContainer
    })
    Create("UIPadding", {
    ...
    -- Helper function to create toggle setting
    local function CreateAnimToggle(name, settingKey, layoutOrder)
        ...
        ToggleBtn.MouseButton1Click:Connect(function()
            ...
            -- Special handling for gradient animations
            if settingKey == "EnableGradients" then
                if enabled then
                    StartAnimationLoop()
                else
                    StopAnimationLoop()
    ...
    CreateAnimToggle("Enable Hover Effects", "EnableHoverEffects", 3)
    CreateAnimToggle("Enable Sounds", "EnableSounds", 4)
    CreateAnimToggle("Enable Particles", "EnableParticles", 5)
    ...
    PanelState.InfoTabContent = Create("ScrollingFrame", {
        ...
        Size = UDim2.new(1, 0, 1, 0),
        Position = UDim2.new(0, 0, 0, 0),
        CanvasSize = UDim2.new(0, 0, 0, 1050),
        ScrollBarThickness = 3,
        ScrollBarImageColor3 = Theme.Accent,
        ScrollBarImageTransparency = 0.3,
        Visible = false,
    ...
    -- Floating animation for Info Header Icon (uses consolidated animation loop)
    local infoIconBaseY = InfoIconContainer.Position.Y.Offset
    local infoIconFloatTime = 0
    local InfoIconFloatConnection = nil
    ...
    local function StartInfoIconFloat()
        ...
        InfoIconFloatConnection = RunService.RenderStepped:Connect(function(dt)
            if InfoIconContainer and InfoIconContainer.Parent then
                ...
                if InfoIconFloatConnection then
                    InfoIconFloatConnection:Disconnect()
                    InfoIconFloatConnection = nil
                end
            end
        end)
    ...
    task.delay(0.8, StartInfoIconFloat)
    ...
    -- Support Contact Card - TsunamiOffical
    local SupportFrame1 = Create("Frame", {
    ...
    Create("UICorner", {CornerRadius = UDim.new(0, 12), Parent = SupportFrame1})
    
    -- Orange accent line
    local Support1AccentLine = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(255, 140, 50),
        Size = UDim2.new(0, 4, 1, -16),
        Position = UDim2.new(0, 8, 0, 8),
        Parent = SupportFrame1
    })
    Create("UICorner", {CornerRadius = UDim.new(0, 2), Parent = Support1AccentLine})
    
    -- Avatar circle
    local Support1Avatar = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(255, 140, 50),
        Size = UDim2.new(0, 40, 0, 40),
        Position = UDim2.new(0, 22, 0.5, -20),
        Parent = SupportFrame1
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = Support1Avatar})
    Create("UIGradient", {
        Color = ColorSequence.new({
    ...
    -- Role tag
    local Support1RoleTag = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(255, 140, 50),
        Size = UDim2.new(0, 60, 0, 16),
        Position = UDim2.new(0, 72, 0, 32),
        Parent = SupportFrame1
    })
    Create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = Support1RoleTag})
    Create("TextLabel", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 1, 0),
        Font = Enum.Font.GothamBold,
        ...
        TextSize = 8,
        Parent = Support1RoleTag
    })
    
    -- Discord contact
    local Support1Discord = Create("Frame", {
        BackgroundTransparency = 1,
    ...
    -- Support Contact Card - zenijux
    local SupportFrame2 = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(20, 20, 30),
    ...
    Create("UICorner", {CornerRadius = UDim.new(0, 12), Parent = SupportFrame2})
    
    -- Green accent line
    local Support2AccentLine = Create("Frame", {
        BackgroundColor3 = Theme.Success,
        Size = UDim2.new(0, 4, 1, -16),
        Position = UDim2.new(0, 8, 0, 8),
        Parent = SupportFrame2
    })
    Create("UICorner", {CornerRadius = UDim.new(0, 2), Parent = Support2AccentLine})
    
    -- Avatar circle
    local Support2Avatar = Create("Frame", {
        BackgroundColor3 = Theme.Success,
        Size = UDim2.new(0, 40, 0, 40),
        Position = UDim2.new(0, 22, 0.5, -20),
        Parent = SupportFrame2
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = Support2Avatar})
    Create("UIGradient", {
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(80, 230, 130)),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(34, 180, 80))
        }),
        Rotation = 135,
        Parent = Support2Avatar
    })
    ...
    -- Role tag
    local Support2RoleTag = Create("Frame", {
        BackgroundColor3 = Theme.Success,
        Size = UDim2.new(0, 60, 0, 16),
        Position = UDim2.new(0, 72, 0, 30),
        Parent = SupportFrame2
    })
    Create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = Support2RoleTag})
    Create("TextLabel", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 1, 0),
        Font = Enum.Font.GothamBold,
        ...
        TextColor3 = Color3.fromRGB(20, 20, 20),
        TextSize = 8,
        Parent = Support2RoleTag
    })
    
    -- Discord contact
    local Support2Discord = Create("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(0, 180, 0, 14),
        Position = UDim2.new(0, 72, 0, 50),
        Parent = SupportFrame2
    })
    Create("TextLabel", {
        BackgroundTransparency = 1,
    ...
    -- Telegram contact
    local Support2Telegram = Create("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(0, 180, 0, 14),
    ...
    -- Lead Developer Card - Modern Glass Design
    ...
    local LeadAccentLine = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(255, 140, 50),
        Size = UDim2.new(0, 4, 1, -16),
        Position = UDim2.new(0, 8, 0, 8),
        Parent = LeadDevFrame
    })
    Create("UICorner", {CornerRadius = UDim.new(0, 2), Parent = LeadAccentLine})
    Create("UIGradient", {
        Color = ColorSequence.new({
        ...
        Rotation = 90,
        Parent = LeadAccentLine
    })
    
    -- Avatar/Badge circle
    local LeadAvatar = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(255, 140, 50),
        Size = UDim2.new(0, 48, 0, 48),
        Position = UDim2.new(0, 22, 0.5, -24),
        Parent = LeadDevFrame
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = LeadAvatar})
    Create("UIGradient", {
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 180, 100)),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 100, 50))
    ...
    -- Role tag
    local LeadRoleTag = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(255, 140, 50),
        Size = UDim2.new(0, 95, 0, 18),
        Position = UDim2.new(0, 82, 0, 38),
        Parent = LeadDevFrame
    })
    Create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = LeadRoleTag})
    Create("TextLabel", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 1, 0),
        Font = Enum.Font.GothamBold,
    ...
    -- Discord with icon
    local LeadDiscord = Create("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(0, 180, 0, 16),
        Position = UDim2.new(0, 82, 0, 60),
        Parent = LeadDevFrame
    })
    Create("TextLabel", {
        BackgroundTransparency = 1,
    ...
    -- ═══════════════════════════════════════════════════════════════
    -- Co-Developer Card - Modern Glass Design
    -- ═══════════════════════════════════════════════════════════════
    local CoDevFrame = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(20, 20, 30),
    ...
    Create("UICorner", {CornerRadius = UDim.new(0, 12), Parent = CoDevFrame})
    
    -- Gradient accent line on left (green)
    local CoAccentLine = Create("Frame", {
        BackgroundColor3 = Theme.Success,
        Size = UDim2.new(0, 4, 1, -16),
        Position = UDim2.new(0, 8, 0, 8),
        Parent = CoDevFrame
    })
    Create("UICorner", {CornerRadius = UDim.new(0, 2), Parent = CoAccentLine})
    Create("UIGradient", {
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(80, 230, 130)),
    ...
    -- Avatar/Badge circle
    local CoAvatar = Create("Frame", {
        BackgroundColor3 = Theme.Success,
        Size = UDim2.new(0, 48, 0, 48),
        Position = UDim2.new(0, 22, 0.5, -24),
        Parent = CoDevFrame
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = CoAvatar})
    Create("UIGradient", {
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(80, 230, 130)),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(34, 180, 80))
    ...
    -- Role tag
    local CoRoleTag = Create("Frame", {
        BackgroundColor3 = Theme.Success,
        Size = UDim2.new(0, 70, 0, 18),
        Position = UDim2.new(0, 82, 0, 36),
        Parent = CoDevFrame
    })
    Create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = CoRoleTag})
    Create("TextLabel", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 1, 0),
        Font = Enum.Font.GothamBold,
    ...
    -- Telegram with icon
    local CoTelegram = Create("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(0, 180, 0, 16),
        Position = UDim2.new(0, 82, 0, 74),
        Parent = CoDevFrame
    })
    Create("TextLabel", {
        BackgroundTransparency = 1,
